...
class TestLoadRunnerAPIWrapper(APITest):
    ...
    def test_start_manuel_start_test_assert_running_wont_start_another_test(self):
        self._setup_manual_test()
        self._assert_setup()
        self.load_runner_api_wrapper.run_from_ui(locust_count=200, hatch_rate=200)
    ...
    def _setup_manual_test(self):
        self.load_runner_api_wrapper.start_manuel_test(self.api_call, self.env, self.node, self.version, self.n_min,
                                                           self.n_max, default_2_cores=True)



    def _setup_and_start_benchmark_test(self):
        self.load_runner_api_wrapper.start_benchmark_test(self.api_call, self.env, self.node, self.version, self.n_min,
                                                           self.n_max, self.n_clients, self.hatch_rate, self.time, False,
                                                          default_2_cores=True)
