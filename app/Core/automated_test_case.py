...
from app.Utils.recurring_event_thread import RecurringEventThread
from app.Core import locust_ui_client
...
class AutomatedTestCase:
    ...
    def __init__(self, setup_name, procedures_name, api_test_runner, config):
        """
        ...
        if self.config.to_failure_procedure_prefix in procedures_name:
    ...
    def __check_and_handle_failure(self, stats):
        if self.__highest_percentage_failure(stats):
            ...
            if self.consecutive_failed_snapshots >= self.config.to_failure_max_consecutive_failed_frames:
                self.failed = True
        else:
