...
class LoadRunner:
    ...
    def setup(cls, config, defualt_2_cores):
        ...
        LoadRunner.RPC_endpoint = config.rpc_endpoint
    ...
    def start_ramp_up(self, locust_count, hatch_rate, first_start=False):
        self.web_client.start_ramp_up(locust_count, hatch_rate, first_start)
    ...
    def __raise_if_running(self):
        old_wait = LocustUIClient.Max_wait
        LocustUIClient.Max_wait = .5
        test_running = self.is_running()
        LocustUIClient.Max_wait = old_wait
        if test_running:
    ...
    def _get_locust_options(self, host, locust_file, log_level, log_file,
        ...
        options = Namespace()
        options.host = host
        options.web_host = self.config.preferred_master
        options.port = self.config.locust_ui_port
        ...
        options.master_host = self.config.preferred_master
        options.master_port = self.config.locust_port
        options.master_bind_host = self.config.preferred_master
        options.master_bind_port = self.config.locust_port
    ...
    def _run_multi_core(self, env_options, master_locust_file, slave_locust_file,
        ...
        # check if more slaves are expected from remote hosts
        if not expected_slaves:
            is_master = True
            self.expected_slaves = available_cores - 1
            env_options.max_slave_index =  self.expected_slaves - 1 #shift to be an index
        elif is_master:
            self.expected_slaves = expected_slaves
            env_options.max_slave_index = available_cores - 2
        else:
            self.expected_slaves = expected_slaves
