...
from app.Data.sql_route_statements import DATA_FACTORY_ROUTES
import requests
import json
from app.Utils.utils import execute_select_statement, execute_commit_statement
...
#TODO: make this come from config
    ...
    def __pull_type_tables(self, ribbon_type, env, include_future = False, **kwargs):
        ...
        if ribbon_type == "rs":
            ...
            users_info = execute_select_statement(self.config, querry, env)
            ...
            rs_user_info = execute_select_statement(self.config, rs_querry, env)
            rs_user_info = [(user[0], int(user[1]), "rs") for user in rs_user_info]
            ls_user_info = execute_select_statement(self.config, ls_querry, env)
    ...
    def __user_guid_taken(self, env, guid):
        ...
        user = execute_select_statement(self.config, querry, env)
        return len(user) == 1

    def __get_user_guids_from_desc(self, env, desc):
        querry = DATA_FACTORY_ROUTES["user_guid_from_desc"].format(desc=desc)
        user_guids = execute_select_statement(self.config, querry, env)
    ...
    def __get_franchise_ids(self, env, count):
        ...
        franchises = execute_select_statement(self.config, querry, env)
        return [franchise[0] for franchise in franchises]

    def __get_user_rule_info(self, env, user_guid):
        url = "{0}{1}".format(self.config.recapi.get_host(env, 1), self.config.recapi.get_route("List Rules", 4))
    ...
    def __get_rec_ids(self, env, count):
        ...
        rec_ids = execute_select_statement(self.config, querry, env)
    ...
    def __delete_users_guid(self, guids_list, env):
        ...
        for user_guid in guids_list:
            ...
            execute_commit_statement(self.config, delete_query, env)
    ...
    def users_count(self, env, desc = None):
        ...
        users = execute_select_statement(self.config, querry, env)
        return len(users)
    ...
    def create_recs(self, env, desc, count, ribbon_type):
        ...
        url = "{host}{route}".format(host=self.config.recapi.get_host(env, 4), route=self.config.recapi.get_route("Create Recordings", 4))
    ...
    def rules_exist(self, min_count, max_count, env, desc):
        ...
        user_info = [(user[0], int(user[1])) for user in execute_select_statement(self.config, querry, env)]
        return self.__get_bad_users(min_count, max_count, user_info)
    ...
    def create_rules(self, env, desc, count):
        ...
        franchise_guids = [fran_guid[0] for fran_guid in execute_select_statement(self.config, querry, env)]
