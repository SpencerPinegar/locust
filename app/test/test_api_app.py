...
class TestAPIApp(TestCase):
    ...
    def __get_start_custom_api_params(self, **kwargs):
        ...
        stat_interval = kwargs["stat_interval"] if "stat_interval" in kwargs.keys() else self.config.stat_reporting_interval
    ...
    def __request_api(self, json):
        web_api_host = self.config.preferred_master
        web_api_port = self.config.rpc_port
        ...
        host = host + self.config.rpc_endpoint
