...
from Load_Test.Misc.utils import get_api_info, obtain_version_number_based_on_weight_func
from Load_Test.Misc.utils import force_route_version_to_ints
from Load_Test.Data.route_relations import APIRoutesRelation
import random
import hashlib
from collections import namedtuple
from Load_Test.Misc.environment_wrapper import APIEnvironmentWrapper as APIWrap
...
class APITasks(TaskSet):
    ...
    def setup_based_on_env_vars(cls):
        ...
        print("GOT ENV VARS AND SETUP -- CREATING DATA")
        #Create data setup functions for task method relation
        setup_relation = cls.__create_setup_relation()
        ...
        for api_call_name, api_call_info in cls.api_info.items():
            ...
            print("Set up {0} Info".format(api_call_name))
    ...
    @classmethod
    def __create_setup_relation(cls):
        ...
        def basic_network_setup():
            ...
            logger.debug("Set up {0} Info".format(APIRoutesRelation.BASIC_NETWORK))
        ...
        def network_byte_size_setup():
            ...
            logger.debug("Set up {0} Info".format(APIRoutesRelation.NETWORK_BYTE_SIZE))

        def small_data_base_setup():
            APITasks.small_db_url = "{host}/rec/test/small-db-test/".format(host=APITasks.api_base_host)
            logger.debug("Set up {0} Info".format(APIRoutesRelation.SMALL_DB))

        def large_data_base_setup():
            APITasks.large_db_url = "{host}/rec/test/big-db-test/".format(host=APITasks.api_base_host)
            logger.debug("Set up {0} Info".format(APIRoutesRelation.LARGE_DB))

        def nginx_check_setup():
            APITasks.nginx_url = "{host}/srv-ok".format(host=APITasks.api_base_host)


        setup_relation = APIRoutesRelation(None, None, None, None, None, None, None, None, None, None, None, None,
                                           nothing_setup, None, basic_network_setup, network_byte_size_setup,
                                           small_data_base_setup, large_data_base_setup, nginx_check_setup)
        return setup_relation

    @staticmethod
    def post_json(locust, url, json_info=None, name=None, assume_tcp_packet_loss=False, bin_by_resp=False):
        """
        ...
        if bin_by_resp:
            APITasks._bin_by_response_time_in_name(locust, url, call_name, header, json_info)
        elif assume_tcp_packet_loss:
            APITasks._assume_tcp_loss_in_name(locust, url, call_name, header, json_info)
        else:
            if json_info:
    ...
    @staticmethod
    def _assume_tcp_loss_in_name(locust, url, call_name, header, json_info):
        if json_info:
            with locust.client.request("POST", url, name=call_name, data=json.dumps(json_info), headers=header,
    ...
    @staticmethod
    def _bin_by_response_time_in_name(locust, url, call_name, header, json_info):
        if json_info:
            with locust.client.request("POST", url, name=call_name, data=json.dumps(json_info), headers=header,
                                       catch_response=True) as resp:
